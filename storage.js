...
6. 每条分析都必须具体、可执行，避免“吸引眼球”“内容丰富”这种泛泛表达。
7. 选题内容必须能直接改写成小红书标题，优先包含人群、场景、痛点或结果承诺。
8. 封面共性必须描述可复用的画面结构、文字结构或信息布局。
...
const DEFAULT_EXPERIMENTAL_SETTINGS = {
  videoCover: false,
...
async function getExperimentalSettings() {
  return {
    ...
    ...(await getStored(STORAGE_KEYS.experimentalSettings, DEFAULT_EXPERIMENTAL_SETTINGS))
  };
}

async function saveExperimentalSettings(settings) {
  const cleaned = {
    videoCover: Boolean(settings.videoCover),
    screenshotFallback: Boolean(settings.screenshotFallback),
    overlayMedia: Boolean(settings.overlayMedia)
  };
  await chrome.storage.local.set({ [STORAGE_KEYS.experimentalSettings]: cleaned });
  return cleaned;
}
...
async function exportBackup() {
  const [notes, categories, aiSettings, updateSettings, uiSettings, promptTemplates, experimentalSettings] = await Promise.all([
    getNotes(),
    getCategories(),
    getAiSettings(),
    getUpdateSettings(),
...
async function importBackup(backup) {
  ...
  await chrome.storage.local.set({
    ...
    [STORAGE_KEYS.experimentalSettings]: {
      ...DEFAULT_EXPERIMENTAL_SETTINGS,
      ...(backup.experimentalSettings || {})
    },
